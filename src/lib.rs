...
fn parse_bounds(bounds: &Punctuated<TypeParamBound, Token![+]>)
-> Option<Conversion> {
    if bounds.len() != 1 { return None }
    if let TypeParamBound::Trait(ref tb) = bounds.first().unwrap().into_value() {
        if let Some(ref seg) = tb.path.segments.iter().last() {
            if let PathArguments::AngleBracketed(ref gen_args) = seg.arguments {
                if gen_args.args.len() != 1 { return None; }
                if let GenericArgument::Type(ref arg_ty) = gen_args.args.first().unwrap().into_value() {
                    if seg.ident == "Into" {
...
fn parse_generics<'a>(decl: &'a FnDecl) -> (HashMap<Ident, Conversion<'a>>, Generics) {
    ...
    let where_clause = if let Some(ref wc) = decl.generics.where_clause {
        ...
        Some(WhereClause { predicates, ..wc.clone() })
    } else {
        None
    };
    (ty_conversions, Generics { params, where_clause, ..decl.generics.clone() })
}
...
fn convert<'a>(
    ...
    inputs.iter().for_each(|input| {
        match input {
            ...
            FnArg::Captured(ArgCaptured { ref pat, ref ty, ref colon_token }) => {
                match *ty {
...
pub fn momo(code: TokenStream, _attr: TokenStream) -> TokenStream {
    ...
    if let Item::Fn(ref item_fn) = fn_item {
        ...
        let (argtypes, mut conversions, argexprs) = convert(&item_fn.decl.inputs, ty_conversions);
        //TODO: add #[inline] attr
